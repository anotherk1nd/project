...
"""
#THIS PLOTS CORRECT VS INCORRECT WITH ORIGINAL VALUES AND THEIR CLASSIFICATIONS
for i in range(len(fs)):
...
for i in range(len(fstest)):
    if fstest[i] == prediction[i]:
        correct, = pl.plot(cscarray[i],datalamarray[i], 'bo')
    if fstest[i] != prediction[i]:
        incorrect, = pl.plot(cscarray[i],datalamarray[i], 'mo')
...
pl.legend([fast_rotators, slow_rotators,correct,incorrect], ['Fast Rotators','Slow Rotators','Correct','Incorrect'])
pl.show()

#I NEED TO PLOT MY PREDICITIONS TO SHOW THEIR SUCCESS HERE
...
print round(clf.score(featurestest,fstest),2)
